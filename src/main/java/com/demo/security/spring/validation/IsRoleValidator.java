 ...
 * - must be non-blank
 * - must start with the prefix 'ROLE_'
 * - must be all upper-case characters
 * - must not contain whitespace
 * - must only contain A-Z, 0-9, and _ characters
 */
public class IsRoleValidator implements ConstraintValidator<IsValidRole, String> {
  ...
  public boolean isValid(String value, ConstraintValidatorContext context) {
    return StringUtils.isNotBlank(value)
        && value.startsWith(ROLE_PREFIX_WITH_UNDERSCORE)
